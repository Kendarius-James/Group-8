...
def become_vendor(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            vendor = Vendor.objects.create(name=user.username,address=user.address,phonenumber=user.phonenumber,company_description=user.company_description, created_by=user)
